...
```sh
clio system regproducer <account> <producer_key> [url] [location] [OPTIONS]
```
...
```sh
clio system regproducer
  <account>                          # REQUIRED: The account to register as a producer
  <producer_key>                     # REQUIRED: The producer's public key
  [url]                              # OPTIONAL: The URL where info about producer can be found
  [location]                         # OPTIONAL: Relative location for purpose of nearest neighbor scheduling
...
The following examples demonstrate how to use the `clio system regproducer` command:

### Register a new producer

```sh
clio system regproducer alice SYS6MRyAjQq8ud7hVNYcfnVPJqcVpscN5So8BhtHuGYqET5GDW5CV https://alice.com 1 -p alice@active
```
