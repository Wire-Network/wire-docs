...
```sh
clio system regproxy <proxy> [OPTIONS]
```
...
```sh
clio system regproxy
  <proxy>                            # REQUIRED: The proxy account to register
...
The following examples demonstrate how to use the `clio system regproxy` command:

### Register an account as a proxy

```sh
clio system regproxy alice -p alice@active
```
