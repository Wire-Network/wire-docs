...
```sh
clio system canceldelay <canceling_account> <canceling_permission> <trx_id> [OPTIONS]
```
...
```sh
clio system canceldelay
  <canceling_account>                 # REQUIRED: Account from authorization on the original delayed transaction
  <canceling_permission>              # REQUIRED: Permission from authorization on the original delayed transaction
  <trx_id>                            # REQUIRED: The transaction id of the original delayed transaction
  ...
  [[-p | --permission] <account@perm>] # an account and permission level to authorize, as in 'account@permission' (defaults to 'canceling_account@canceling_permission')
...
The following examples demonstrate how to use the `clio system canceldelay` command:

### Cancel a delayed transaction

```sh
clio system canceldelay alice active 1234567890abcdef1234567890abcdef1234567890abcdef1234567890abcdef -p alice@active
```
