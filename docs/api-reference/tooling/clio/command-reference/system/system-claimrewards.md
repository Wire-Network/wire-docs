...
```sh
clio system claimrewards <owner> [OPTIONS]
```
...
```sh
clio system claimrewards
  <owner>                            # REQUIRED: The account to claim rewards for
  ...
  [[-p | --permission] <account@perm>] # an account and permission level to authorize, as in 'account@permission' (defaults to 'owner@active')
...
The following examples demonstrate how to use the `clio system claimrewards` command:

### Claim producer rewards

```sh
clio system claimrewards alice -p alice@active
```
