...
```sh
clio get actions
  account_name <TEXT>     # REQUIRED: name of account to query on
  pos <INT>               # OPTIONAL: sequence number of action for this account, -1 for last
  offset <INT>            # OPTIONAL: get actions [pos, pos + offset] for positive offset or [pos - offset, pos] for negative offset
