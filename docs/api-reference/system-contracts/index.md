...
Below are listed the actions which are declared in the `sysio.bios` contract, mapped one-to-one with the native EOSIO actions, but having no implementation at the contract level:
...
The workflow to propose, review, approve and then executed a transaction is describe in details [here](./guides/how-to-sign-a-multisig-transaction-with-sysio.msig.md), and in short it can be described by the following:
...
The answer to this question is explained in detailed [here](./guides/how-to-use-sysio.wrap.md)
