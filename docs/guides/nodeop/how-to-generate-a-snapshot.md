...
* Ensure that the [`producer_api_plugin`](/docs/api-reference/tooling/nodeop/plugins/producer-api-plugin.md) is enabled on your `nodeop` instance and Producer API is accessible via HTTP on port 8887. If you followed standard installation with `wire-cli`, that plugin is already enabled and configured at `/opt/wire-network/blockproducer/config/config.ini`.
