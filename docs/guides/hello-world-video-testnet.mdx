...
- Confirm the new contract policy was created. In the [Wire Hub](https://hub.wire.network) or [Explorer](https://hub.wire.network/explore), you should see:
   ...
   - **Public Keys**: Matching the public key you just saved.
...
### 4. Import the private key (from your contract account)
...
:::info
Don't forget adding the `-u` to the deploy script. The video was recorded on a server already connected to the testnet locally, hence it was unintentionally omitted.
:::
